require("prototypes.particles.incendiary-trail")

require("prototypes.projectiles.incendiary-pellet")

require("prototypes.items.sniper-rifle")
require("prototypes.recipes.sniper-rifle")
require("prototypes.items.coilgun")
require("prototypes.recipes.coilgun")

require("prototypes.items.explosive-rounds-magazine")
require("prototypes.recipes.explosive-rounds-magazine")
require("prototypes.items.incendiary-shotgun-shell")
require("prototypes.recipes.incendiary-shotgun-shell")

require("data_raw_edits")
